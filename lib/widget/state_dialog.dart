...
Future showStateDialog(
    {required BuildContext context, required bool isSuccess, required String text, bool closeOnConfirm = false}) {
  return showDialog(
...
Future showLottieDialog(
    {required BuildContext context, required String text, required String asset, bool closeOnConfirm = false}) {
  return showDialog(
    barrierDismissible: false,
    context: context,
    builder: (context) => _LottieDialog(contentText: text, asset: asset, closeOnConfirm: closeOnConfirm),
  );
}

class _LottieDialog extends HookWidget {
  const _LottieDialog({Key? key, required this.contentText, required this.asset, required this.closeOnConfirm})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      content: Column(
        ...
        children: [
          Lottie.asset(asset, height: 100, width: 100, fit: BoxFit.contain, controller: controller, onLoaded: (p0) {
            controller
