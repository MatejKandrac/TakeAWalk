...
class AppScaffold extends StatelessWidget {
  const AppScaffold({Key? key, required this.appBar, required this.navigationIndex, required this.child, this.fab})
      : super(key: key);
  ...
  void _onNavigationSelected(int value, BuildContext context) {
    ...
    AutoRouter.of(context).pushAndPopUntil(nextRoute,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        decoration:
            const BoxDecoration(boxShadow: [BoxShadow(color: Color(0xff000000), blurRadius: 10, offset: Offset(0, 5))]),
        child: NavigationBar(
          onDestinationSelected: (value) => _onNavigationSelected(value, context),
          selectedIndex: navigationIndex,
          destinations: const [
            NavigationDestination(icon: Icon(Icons.calendar_month), label: "My events"),
            NavigationDestination(icon: Icon(Icons.list), label: "Invites"),
